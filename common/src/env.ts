...
function init_env(): Env {
  ...
  return {
    APARTMENT: expect(
      process.env.APARTMENT,
      '.env file must specify "APARTMENT"'
    ),
    MQTT_HOST: expect(
      process.env.MQTT_HOST,
      '.env file must specify "MQTT_HOST"'
    ),
...
function parseLogLevel(): LogLevelDesc {
  ...
  if (typeof logLevel === "undefined") {
    ...
    throw new Error(
      `env must specify valid "LOG_LEVEL" (${levels}), got "${logLevel}"`
    );
  }
}
...
function parseMqttAuth(): Auth | undefined {
  ...
  if (user === undefined && pass === undefined) {
    ...
    throw new Error(
      `.env file must also specify "MQTT_USER" if "MQTT_PASS" is specified`
    );
  }
}
