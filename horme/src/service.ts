...
import { Subscription, util, env as getEnv } from "horme-common";
...
async function removeService(uuid: string) {
  ...
  const removals = previousServices.filter(
    (prev) => !newServices.includes(prev.uuid)
  );
...
async function configureService(service: Service, depends: Uuid[]) {
  ...
  if (reconfigure) {
    ...
    await client.publish(topic, JSON.stringify({ add, del }), {
      qos: 2,
      retain: true,
    });
